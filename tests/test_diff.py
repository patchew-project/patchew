...
class DiffTest(PatchewTestCase):
    ...
    def test_rest_other_versions(self):
        ...
        resp2 = self.api_client.get(
            self.PROJECT_BASE + "series/20160628014747.20971-2-famz@redhat.com/"
        )
        resp3 = self.api_client.get(
            self.PROJECT_BASE + "series/20160628014747.20971-3-famz@redhat.com/"
        )
        self.assertEqual(resp1.data["other_versions"][0]["version"], 2)
        self.assertEqual(
            resp1.data["other_versions"][0]["resource_uri"], resp2.data["resource_uri"]
        )
        self.assertEqual(resp1.data["other_versions"][1]["version"], 3)
        self.assertEqual(
            resp1.data["other_versions"][1]["resource_uri"], resp3.data["resource_uri"]
        )
        self.assertEqual(resp2.data["other_versions"][0]["version"], 1)
        self.assertEqual(
            resp2.data["other_versions"][0]["resource_uri"], resp1.data["resource_uri"]
        )
        self.assertEqual(resp2.data["other_versions"][1]["version"], 3)
        self.assertEqual(
            resp2.data["other_versions"][1]["resource_uri"], resp3.data["resource_uri"]
        )
        self.assertEqual(resp3.data["other_versions"][0]["version"], 1)
        self.assertEqual(
            resp3.data["other_versions"][0]["resource_uri"], resp1.data["resource_uri"]
        )
        self.assertEqual(resp3.data["other_versions"][1]["version"], 2)
        self.assertEqual(
            resp3.data["other_versions"][1]["resource_uri"], resp2.data["resource_uri"]
        )
