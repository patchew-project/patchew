...
class Converter(metaclass=abc.ABCMeta):
    ...
    def get_projects_with_tests(self):
        return self.Project.objects.filter(
            projectproperty__name__startswith="testing."
        ).distinct()

    def get_tests(self, po):
        # Based on TestingModule.get_tests
        ret = {}
        props = self.ProjectProperty.objects.filter(
            name__startswith="testing.tests.", project=po
        )
        for p in props:
            k = p.name
            v = load_property(p)
        ...
        for po in self.Project.objects.all():
            ...
            for obj in self.get_objects_for_project(po):
                ...
                for prop in self.get_properties(
                    obj, name__startswith="testing.report."
                ):
                    ...
                    r.name = "testing." + tn
                    r.status = (
                        api.models.Result.SUCCESS
                        if passed
                        else api.models.Result.FAILURE
                    )
    ...
    def do_result_to_properties(self):
        for po in self.Project.objects.all():
            for obj in self.get_objects_for_project(po):
                ...
                if (
                    by_status[api.models.Result.RUNNING]
                    + by_status[api.models.Result.PENDING]
                ):
...
class Migration(migrations.Migration):
    ...
    dependencies = [("api", "0028_populate_git_results")]

    operations = [
        migrations.RunPython(result_from_properties, reverse_code=result_to_properties)
    ]
