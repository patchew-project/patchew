...
class DiffModule(PatchewModule):
    ...
    def get_other_versions(self, message, request, format):
        ...
        return [
            {
                "version": o.version,
                "resource_uri": rest_framework.reverse.reverse(
                    "series-detail",
    ...
    def prepare_message_hook(self, request, message, for_message_view):
        ...
        html = "Diff against"
        for v, url in self.get_other_versions_urls(
            message.project, message.message_id, other_versions
        ):
    ...
    def www_url_hook(self, urlpatterns):
        urlpatterns.append(
            url(
                r"^(?P<project>[^/]*)/(?P<series_left>.*)/diff/(?P<series_right>.*)/",
